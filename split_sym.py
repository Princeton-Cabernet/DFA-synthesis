...
is_split=lambda dst:(len(out_edges[dst])<=split_degree_thres) and (len(in_symbols[dst])>1)
...
for dst in states:
    if is_split(dst):
        sys.stderr.write(f'notice: state {dst} is split into {len(in_symbols[dst])} nodes.\n')
        split_nodes[dst]=[(dst, sym) for sym in in_symbols[dst]]
    else:
        split_nodes[dst]=[(dst, None)]
    expanded_states.update(split_nodes[dst])
def map_dst_state_to_tup(dst, sym):
    if is_split(dst):
        return (dst,sym)
    return (dst, None)
for _,sym,dst in transitions:
    assert(map_dst_state_to_tup(dst,sym) in expanded_states)
...
for (src_state, symbol, dst_state) in transitions:
    for split_src in split_nodes[src_state]: # all split src nodes need to transition correctly
        split_dst=map_dst_state_to_tup(dst_state, symbol) #if dst is split, only one node matters
